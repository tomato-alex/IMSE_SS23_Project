...
CREATE TABLE leasing(
    ...
    CONSTRAINT duration_check CHECK (duration >= 0 AND duration <= 20),
    CONSTRAINT fee_check CHECK (fee >= 0),    
...
SELECT car.brand, car.modell, leasing.duration, leasing.fee
FROM car JOIN leasing
ON car.leasingNr  = leasing.leasingNr;




CREATE VIEW total_sales AS
    SELECT employee.employeeId, employee.first_name, employee.last_name, car.brand, car.modell,  sells.date, SUM(price) sum
    FROM sells JOIN employee
                      ON employee.employeeId = sells.employeeId
    JOIN Car
    ON car.carId = sells.carId
GROUP BY employee.employeeId, employee.first_name, employee.last_name, car.brand, car.modell,sells.date
