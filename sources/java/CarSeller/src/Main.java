...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= 200; ++i) {
            int random = rand.nextInt(aid.size());
            while (used.contains(random)) { // if a number that has been already used occurs, use random function until
                                            // unused number arrives so that there will not be unique constraint
                                            // violation
                random = rand.nextInt(aid.size());
            }
            used.add(random);
        ...
        for (int i = 0; i < fid.size(); ++i) { // workers without bosses
            ...
            helper.insertIntoMitarbeiter(dgen.getFirstName(randomName), dgen.getSurname(randomSurname), fid.get(i),
                    null); // in every location/filiale will be one boss
        }

        {
            ...
            for (int i = 0; i < fid.size(); ++i) {
                for (int j = 0; j < 5; ++j) {
                    int randomName = rand.nextInt(dgen.getFirstNameSize());
                    int randomSurname = rand.nextInt(dgen.getSurnameSize());
                    helper.insertIntoMitarbeiter(dgen.getFirstName(randomName), dgen.getSurname(randomSurname),
                            fid.get(i), filialAndMitarbeiter.get(i));
                }
            }
        }
        ...
        for (int i = 0; i < 500; ++i) {
            ...
            String date = (rand.nextInt(2022 - 2012) + 2012) + "-" + (rand.nextInt(12) + 1) + "-"
                    + (rand.nextInt(28) + 1);
