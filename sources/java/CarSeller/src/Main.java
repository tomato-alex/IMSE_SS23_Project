...
public class Main {
        public static void main(String[] args) {
                ...
                for (int i = 0; i < 100; ++i) {
                        ...
                        helper.insertIntoLocation(dgen.getCity(getter), dgen.getCountry(getter),
                                        dgen.getRandomAddress());
                }
                ...
                for (int i = 0; i < locationId.size(); ++i) { // workers without bosses
                        ...
                        helper.insertIntoEmployee(dgen.getFirstName(randomName), dgen.getSurname(randomSurname),
                                        locationId.get(i), null); // in every location/filiale will be one boss
                }

                {
                        ...
                        for (int i = 0; i < locationId.size(); ++i) {
                                for (int j = 0; j < 5; ++j) {
                                        int randomName = rand.nextInt(dgen.getFirstNameSize());
                                        int randomSurname = rand.nextInt(dgen.getSurnameSize());
                                        helper.insertIntoEmployee(dgen.getFirstName(randomName),
                                                        dgen.getSurname(randomSurname), locationId.get(i),
                                                        filialAndMitarbeiter.get(i));
                                }
                        }
                }
